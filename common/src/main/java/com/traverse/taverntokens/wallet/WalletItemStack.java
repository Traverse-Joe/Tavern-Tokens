...
public class WalletItemStack extends ItemStack implements WalletItemStackInterface {
    ...
    public WalletItemStack(Holder<Item> entry) {
        this((ItemLike)entry.value(), 1);
    }

    public WalletItemStack(Holder<Item> itemEntry, long count) {
    ...
    public boolean isEmpty() {
        return this == EMPTY || this.getItem().equals(Items.AIR) || this.count <= 0;
    }
    ...
    public static WalletItemStack fromNbt(CompoundTag nbt) {
        try {
    ...
    public boolean isStackable() {
        return getMaxItemCount() > 1 && (!this.isDamageableItem() || !this.isDamaged());
    }

    @Override
    public WalletItemStack copy() {
        if (this.isEmpty()) return EMPTY;
        WalletItemStack itemStack = new WalletItemStack(this.getItem(), this.count);
        itemStack.setPopTime(this.getPopTime());
        if (this.getTag() != null) itemStack.setTag(this.getTag().copy());
        return itemStack;
    }
    ...
    public static boolean canCombine(ItemStack stack, ItemStack otherStack) {
        if (!stack.is(otherStack.getItem())) return false;
